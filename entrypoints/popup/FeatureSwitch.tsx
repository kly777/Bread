...
const FeatureSwitch: Component<FeatureSwitchProps> = (props) => {
	const [settingState, setSettingState] = createSignal<SettingState>("default");

	// 获取 domain 并创建 featureSettingStorage 实例
	const getFeatureStorage = () => {
		const currentDomain = getDomain();
		console.log("domain:", currentDomain);
		return new featureSettingStorage(props.featureName, currentDomain);
	};
	...
	const handleRadioChange = async (value: SettingState) => {
		...
		try {
			...
			console.warn(`写入${props.featureName}存储配置失败`, error);
		}
	};
	...
	return (
		<div class="feature-switch">
			<div class="feature-header">
				<span class="feature-name">{props.featureName}</span>
