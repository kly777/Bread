...
interface AIConfigProps {
    onConfigChange?: (config: { provider: 'openai' | 'deepseek' | 'mock'; config: { apiKey: string; endpoint: string; model: string; maxTokens: number; temperature: number } }) => void
}
...
const AIConfig: Component<AIConfigProps> = (props) => {
    const [selectedProvider, setSelectedProvider] = createSignal<AIProvider['id']>('mock')
    ...
    const saveConfig = async () => {
        try {
            ...
            await browser.storage.local.set({ 'configs:ai': config })
            ...
            <Show when={!isLoading()}>
                ...
                <div class="ai-config-section">
                    ...
                    <div class="ai-providers">
                        {AI_PROVIDERS.map(provider => (
                            <div 
                                class="ai-provider"
                                classList={{ 'ai-provider-selected': selectedProvider() === provider.id }}
                                onClick={() => {
                    ...
                    <div class="ai-config-grid">
                        ...
                        <div class="ai-config-section">
                            ...
                            <input
                                ...
                                step="0.1"
                                value={temperature()}
                                onInput={(e) => setTemperature(parseFloat(e.currentTarget.value) || 0.7)}
                            />
                            <div class="ai-config-hint">
                                控制随机性 (0-2)
                            </div>
                ...
                <Show when={selectedProvider() === 'mock'}>
                    <div class="ai-config-mock-info">
                        <div class="ai-config-mock-icon">🎭</div>
                        <div class="ai-config-mock-text">
                            <strong>模拟模式已启用</strong>
                            <p>此模式用于开发和测试，无需API密钥即可体验AI功能。</p>
                            <p>要使用真实的AI服务，请选择OpenAI或DeepSeek提供者。</p>
                        </div>
                    </div>
                </Show>
                ...
                <Show when={selectedProvider() === 'deepseek'}>
                    <div class="ai-config-tip">
                        <strong>DeepSeek使用提示:</strong>
                        <ul>
                            <li>DeepSeek API兼容OpenAI格式，可直接使用</li>
                            <li>需要申请API密钥: <a href="https://platform.deepseek.com/" target="_blank" rel="noopener noreferrer">DeepSeek平台</a></li>
                            <li>支持模型: deepseek-chat (非思考模式), deepseek-reasoner (思考模式)</li>
                            <li>也可使用基础URL: https://api.deepseek.com (不带/v1)</li>
                        </ul>
                    </div>
                </Show>
            </Show>
