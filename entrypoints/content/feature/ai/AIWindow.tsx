...
const AIWindow: Component<AIWindowProps> = (props) => {
    ...
    onMount(() => {
        if (dragHandleRef) {
            dragHandleRef.addEventListener('mousedown', handleDragStart)
        }
        ...
        return () => {
            if (dragHandleRef) {
                dragHandleRef.removeEventListener('mousedown', handleDragStart)
            }
            ...
            {!isMinimized() && (
                <div class="bread-ai-content">
                    ...
                    <div class="bread-ai-section">
                        <div class="bread-ai-section-title">网页内容分析</div>
                        <button
                            class="bread-ai-btn bread-ai-btn-primary"
                            onClick={props.onSummarize}
                            disabled={props.isLoading}
                        >
                            {props.isLoading ? '分析中...' : '总结当前页面'}
                        </button>
                    </div>

                    {/* 重点标记工具 */}
                    <div class="bread-ai-section">
                        <div class="bread-ai-section-title">重点标记工具</div>
                    ...
                    {(props.result || props.isLoading) && (
                        <div class="bread-ai-section">
                            <div class="bread-ai-section-title">分析结果</div>
                            <div
                                class="bread-ai-result"
                                classList={{
                                    'bread-ai-result-error': props.isError,
                                    'bread-ai-result-loading': props.isLoading
                                }}
                            >
                                {props.isLoading ? (
