...
export function UI(props: UIProps) {
	...
	const [history, setHistory] = createSignal<
		{ command: string; result: string; timestamp: Date }[]
	>([]);
	...
	const handleSubmitStreaming = async (currentCommand: string) => {
		...
		try {
			...
			setStreamingResult(
				`流式执行失败: ${error instanceof Error ? error.message : String(error)}`,
			);
			setResult(
				`执行失败: ${error instanceof Error ? error.message : String(error)}`,
			);
	...
	const handleGetPageState = async () => {
		...
		try {
			...
			setResult(
				`获取页面状态失败: ${error instanceof Error ? error.message : String(error)}`,
			);
		} finally {
			setIsLoading(false);
		}
							...
							<div style={{ display: "flex", gap: "10px" }}>
								...
								<button
									...
									style={{
										padding: "12px 20px",
										background:
											isLoading() || isStreaming() ? "#6c757d" : "#007bff",
										color: "white",
										border: "none",
										"border-radius": "8px",
										cursor:
											isLoading() || isStreaming() ? "not-allowed" : "pointer",
										"font-size": "14px",
										"font-weight": "bold",
									}}
								>
