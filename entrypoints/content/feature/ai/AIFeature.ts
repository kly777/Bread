...
interface SimpleAIService {
    ...
    invokeStream(messages: unknown[], tools?: unknown[]): Promise<ReadableStream<unknown>>;
...
export class AIFeature extends Feature {
    ...
    async on(): Promise<void> {
        ...
        try {
            ...
            // 创建工具执行器
            this.toolExecutor = new ToolExecutor({ pageController: this.pageController });
    ...
    async off(): Promise<void> {
        ...
        import('./UI').then(({ destroyUI: destroyUI }) => {
            destroyUI();
        }).catch((error) => {
    ...
    async executeCommand(command: string): Promise<string> {
        if (!this.isActive || !this.pageController || !this.aiService || !this.toolExecutor) {
    ...
    private async processCommandWithAIStream(command: string): Promise<ReadableStream<string>> {
        if (!this.pageController || !this.aiService || !this.toolExecutor) {
            return new ReadableStream<string>({
                start(controller) {
        ...
        try {
            ...
            return new ReadableStream<string>({
                start(controller) {
                    controller.enqueue(`AI 处理失败: ${error instanceof Error ? error.message : String(error)}`);
                    controller.close();
                }
            });
        }
        ...
        if (tool_calls && tool_calls.length > 0) {
            ...
            for (const toolCall of tool_calls) {
                ...
                if (type === 'function' && func) {
                    ...
                    try {
                        const toolResult = await this.toolExecutor.executeToolCall(name, argsStr);
                        toolResults.push(`工具调用 ${name}: ${toolResult.message}`);
                    } catch (error) {
                        toolResults.push(`工具调用 ${name} 失败: ${error instanceof Error ? error.message : String(error)}`);
                    }
                }
            }
    ...
    private handleAIStream(stream: ReadableStream<unknown>): ReadableStream<string> {
                    ...
                    while (true) {
                        ...
                        if (type === 'content') {
                            ...
                            if (!actualToolName) {
                                ...
                                const functionData = toolData.function as Record<string, unknown> | undefined;
                            ...
                            if (toolExecutor && typeof data === 'object' && data !== null) {
                                ...
                                if (functionData?.name && functionData?.arguments) {
                                    ...
                                    try {
                                        const toolResult = await toolExecutor.executeToolCall(name, argsStr);
                                        controller.enqueue(`[工具执行结果: ${toolResult.message}]`);
                                    } catch (error) {
                                        controller.enqueue(`[工具执行失败: ${error instanceof Error ? error.message : String(error)}]`);
                                    }
                                }
                            }
                        } else if (type === 'finish') {
    ...
    async executeCommandStreamWithAutoLoop(command: string): Promise<ReadableStream<string>> {
        // 简化实现：直接返回普通流式响应
