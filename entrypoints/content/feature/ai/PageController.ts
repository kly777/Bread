...
export class PageController {
	...
	async inputText(index: number, text: string): Promise<ActionResult> {
		try {
			const element = getElementByIndex(getInteractiveElements(), index) as
				| HTMLInputElement
				| HTMLTextAreaElement;
			if (!element) {
				return {
	...
	async selectOption(index: number, value: string): Promise<ActionResult> {
		try {
			const element = getElementByIndex(
				getInteractiveElements(),
				index,
...
function buildPageHeader(
	...
	const scrollHintAbove =
		hasContentAbove && config.viewportExpansion !== -1
...
function buildPageFooter(
	...
	const scrollHintBelow =
		hasContentBelow && config.viewportExpansion !== -1
