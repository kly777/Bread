...
class BatchTranslator {
  ...
  private async processSingle(item: BatchTranslationItem): Promise<void> {
    try {
      const result = await this.performSingleTranslation(item.originalText, item.targetLang);
      item.resolve(result);
    } catch (error) {
      item.reject(error);
    }
  ...
  private async processBatchRequest(batch: BatchTranslationItem[]): Promise<void> {
    ...
    const promises: Promise<BatchResult>[] = batch.map(item =>
      this.performSingleTranslation(item.originalText, item.targetLang)
        .then(result => ({ item, result } as BatchResult))
        .catch(error => ({ item, error } as BatchResult))
    );
    ...
    results.forEach(result => {
      if (result.status === 'fulfilled') {
        ...
        console.warn('Unexpected promise rejection in batch translation');
      }
    });
  }

  private async performSingleTranslation(originalText: string, targetLang: string): Promise<string> {
