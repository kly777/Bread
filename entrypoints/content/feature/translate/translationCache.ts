...
class TranslationCacheManager {
	...
	async initialize(): Promise<void> {
		...
		try {
			...
			const cacheData = result[cacheKey] as CacheData | undefined;
			...
			if (cacheData) {
				...
				// 如果当前域名没有缓存，创建一个空的缓存
				this.memoryCaches.set(domain, new Map<string, CacheEntry>());
				console.log(`域名 ${domain} 的翻译缓存初始化完成（无历史缓存）`);
			}
		} catch (error) {
	...
	private async saveDomainCache(domain: string): Promise<void> {
		...
		try {
			...
			console.warn(`保存域名 ${domain} 的翻译缓存失败:`, error);
		}
	}
	 ...
	 * 获取指定域名的缓存统计信息
	 */
	getDomainStats(domain: string): { size: number; maxSize: number } | null {
		const domainCache = this.memoryCaches.get(domain);
		if (!domainCache) return null;
	...
	async clearDomain(domain: string): Promise<void> {
		...
		try {
			...
			console.warn(`清空域名 ${domain} 的翻译缓存失败:`, error);
		}
	}
}
