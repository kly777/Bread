...
export function createAnchorLayout(): HTMLElement {
	...
	const calculatePageStats = () => {
		try {
			...
			pageWordCount = textContent.split(/\s+/).filter(Boolean).length;
	...
	const updateScrollProgress = () => {
		...
		scrollProgress = maxScroll <= 0 ? 0 : (scrollTop / maxScroll) * 100;
	};
	...
	const updateUI = () => {
		// 页面统计
		if (pageWordCount > 0) {
			pageStatsSpan.textContent = `pw: ${(pageWordCount / 1000).toFixed(
				1,
